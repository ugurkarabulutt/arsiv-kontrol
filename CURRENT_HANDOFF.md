...
- Yerelde mevcut `Geri Bildirim Merkezi` ve `Geçmiş Düzeltme` akışı, kalan açık işleri aynı
  ekranda görebilecek şekilde güçlendirildi; her sorun için yeni ekran açılmadı. `/api/alerts`
  artık son uyarı limitine takılmadan açık feedback kayıtlarını ayrıca getirir ve her feedback
  için çalışma türü döndürür: `Düzeltmeye hazır`, `Çoklu geçiş`, `Eksik düzeltme`,
  `Yanlış alarm`, `Eşleşme yok`, `Yorum / düzen`, `Metin doğru`. Admin feedback ekranında bu
  iş türlerine göre filtreleme yapabilir. Uygun yanlış-düzeltme feedback kartlarında
  `Düzeltmeye Hazırla` butonu mevcut `Geçmiş Düzeltme` formunu bildirilen doküman kapsamıyla
  doldurur; otomatik geçmiş/geniş düzeltme yapmaz. Çoklu geçişlerde admin yine etki taramasından
...
- Canlı Supabase feedback kuyruğunda güvenli düzeltme temizliği yapıldı. Kullanıcı kararı gereği
  ...
  düzeltildi. Başlangıçta `alerts(type='feedback')` içinde 611 feedback vardı; 180 açık,
  431 çözülmüş durumdaydı. Mevcut düzeltme paketlerinden güvenli olanlar ve metinde karşılığı net
  bulunan yeni küçük paketler işlendi. Son canlı doğrulama: 611 toplam feedback, 496 çözülmüş,
  115 açık; `content_correction_packages` 56 paket; `Codex Admin Feedback Cleanup` tarafından
  yazılmış 56 geri alınabilir `content_correction_log` kaydı var. İşlem öncesi paket yedekleri
  `settings` altında saklandı: `content_correction_packages_backup_codex_20260817141108`,
  `content_correction_packages_backup_codex_20260817142010`,
  `content_correction_packages_backup_codex_20260817142336`. Özellikle `TEVBE 1.` başlıkları
  bildirilen kayıtlarla sınırlı biçimde `TÖVBE 1.` yapıldı; `Tevbe 9` gibi ayet/sure atıflarına
  dokunulmadı. Açık kalan paket bağlantılı iki konu manuel kontrol gerektiriyor:
  `Ahlaki kelimesi metne özel koruma` ve `Yakîn kelimesi düzeltmesi`. Public ön yüz/root cutover
  veya kod deploy yapılmadı; bu adım canlı veri düzeltme operasyonudur.

- Aynı gün açık feedback kuyruğu ikinci kez işlendi. Bu geçişte yalnız `Yanlış düzeltme`
  feedback'lerinde, düzeltilmiş metinde hedef ifade tam 1 kez geçiyorsa canlı kayıtta mevcut
  denetim kaydına özel geri dönüş uygulandı. 45 kayıt grubu, 46 feedback ve 45 metin düzeltmesi
  işlendi; `content_correction_packages_backup_codex_20260817144845` yedeği alındı. Son canlı
  durum: 611 toplam feedback, 542 çözülmüş, 69 açık; `content_correction_packages` 101 paket,
  `Codex Admin Feedback Cleanup` için 101 geri alınabilir `content_correction_log` kaydı.
  Çoklu geçiş, eksik hata, yanlış alarm ve düzen yorumu içeren kalan 69 kayıt otomatik
  değiştirilmedi; admin çalışma listesi `docs/feedback-open-triage-2026-08-17.md` dosyasına
  kaydedildi. Bu adımda da benzer geçmiş kayıtlara, public ön yüze, root cutover'a veya deploy'a
  dokunulmadı.
...
- Excel soru aktarımı ikinci kez kökten güçlendirildi. `/admin > Arşiv Operasyon Merkezi >
  ...
  header'ları noindex/no-store doğru; oturumsuz soru aktarımı başlangıç endpoint'i `401`
  döndü. Canlı Excel soru aktarımı arka plandan tamamlandı: `Arşiv Data.xlsx` listesindeki
  2.800 uygulanmış eşleşmenin 2.798'inde soru alanı dolu; 389 boş soru alanı dolduruldu,
  2 kayıt Excel'de soru hücresi boş geldiği için güvenle uydurulmadı.
...
- Canlıda Excel soru aktarımı ekran kapanmasına dayanıklı hale getirildi. `/admin > Arşiv
  ...
  devam edebilir bir iş olarak başlatılır; frontend yalnız durumu izler. Kullanıcı sayfadan
  ayrılırsa işlem kaydı korunur, geri dönünce durum otomatik kontrol edilir ve gerekirse iş
  kaldığı yerden yeniden tetiklenir. Bağlantı kopması artık doğrudan kırmızı hata penceresine
  düşmez; sistem bekleyen aktarımı hatırlar. Yeni endpoint'ler:
  `POST /api/history-tags/import-batches/:id/backfill-questions/start` ve
...
- Yerelde etiket aktarımı toplu uygulama canlı şema uyumsuzluğu düzeltildi. Canlıda
  ...
  `678e8c2` GitHub'a push edildi ve production'a alındı. Production deploy:
  `https://arsiv-kontrol-d3x39k8ee-ugurkarabulutts-projects.vercel.app`, canlı alias
  `https://arsiv.ibrahimlive.ai`. Canlı smoke: `/health`, root `/`, `/admin`, `/admin/`,
  `/admin/smoke-test`, `/api/auth/me` başarılı; `/admin` header'ları noindex/no-store doğru.
  Oturumsuz toplu uygulama endpoint'i `401` döndü. Canlı HTML'de
...
- Yerelde etiket aktarımı `Kontrol gerekenler` grubu için toplu uygulama aksiyonu eklendi.
  ...
  `node --check server.js`, `node scripts/check-frontend.js`, `git diff --check` ve
  `npm.cmd run check` başarılı; 86/86 test geçti. Runtime commit `d3fb222` GitHub'a push
  edildi ve production'a alındı. Production deploy:
...
- Yerelde etiket aktarımı kullanıcı dili sadeleştirildi. `/admin > Arşiv Operasyon Merkezi >
  ...
  Yerel doğrulama: `node --check server.js`, `node scripts/check-frontend.js`,
  `git diff --check` ve `npm.cmd run check` başarılı; 86/86 test geçti. Runtime commit
  `0956960` GitHub'a push edildi ve proje notlarına işlendi. Production deploy:
  `https://arsiv-kontrol-6889s8jsp-ugurkarabulutts-projects.vercel.app`, canlı alias
  `https://arsiv.ibrahimlive.ai`. Canlı smoke: `/health`, root `/`, `/admin`, `/admin/`,
  `/admin/smoke-test`, `/api/auth/me`, manifest, `sw.js` ve favicon başarılı. `/admin`
  header'ları noindex/no-store doğru. Canlı HTML'de `Güvenli Eşleşmeleri Uygula`,
...
- Canlıda etiket aktarımı opsiyonel `history` kolonlarına dayanıklı hale getirildi. Büyük
  ...
  `column history.prompt_version does not exist` hatası oluştu. Etiket aktarımı artık
  `original_text`, `tags`, `prompt_version` ve `rules_hash` gibi sonradan eklenen kolonları
  canlı şema tespitine göre seçer; olmayan kolonları sorguya eklemez. Kapsam: `server.js`,
  `scripts/check-frontend.js`. SQL/DB migration gerekmedi; root `/` public cutover ve public
  frontend hattına dokunulmadı. Yerel doğrulama: `node --check server.js`,
  `node scripts/check-frontend.js`, `git diff --check` ve `npm.cmd run check` başarılı;
  86/86 test geçti. Runtime commit `75d2a55` GitHub'a push edildi ve production'a alındı.
  Production deploy: `https://arsiv-kontrol-1et3fa4c5-ugurkarabulutts-projects.vercel.app`,
  ...
  oturumsuz upload start endpoint'i `401` döndü ve eski geçici `adminRouteProbe` yok.
...
- Yerelde Public JSON kalite kapisi eklendi: `/admin` super admin `Arsiv Operasyon Merkezi >
  ...
  `server.js`, `index.html`, `scripts/check-frontend.js`, `AGENTS.md`, `CURRENT_HANDOFF.md`.
  DB/schema/root `/` ve public frontend hattina dokunulmadi. Yerel dogrulama:
  `npm.cmd run check` basarili, 86/86 test gecti. Runtime commit `23d101e` GitHub'a push
  edildi ve production'a alindi. Production deploy:
