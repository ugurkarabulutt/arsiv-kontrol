-- Keep every review list in latest-workflow order without rewriting history rows.
create index if not exists history_revisions_status_transition_idx
  on public.history_revisions (history_id, created_at desc, version desc)
  where (before_data->>'status') is distinct from (after_data->>'status');

create index if not exists admin_action_log_status_transition_idx
  on public.admin_action_log (target_id, created_at desc, id desc)
  where target_type = 'history'
    and target_status_before is distinct from target_status_after;
...
comment on view public.review_history_queue is
  'Private read-only review queue. Pending uses latest submission; every other status uses latest workflow activity with preserved historical fallbacks.';
