...
assert(server.includes("app.post('/api/newsletter/subscribe', publicNewsletterRateLimiter, publicArchiveNewsletterSubscribeHandler)"), 'Root public abonelik endpointi bayrakli olarak tanimli olmali.');
...
for (const marker of ['PUBLIC_ARCHIVE_SEMANTIC_SEARCH_ENABLED', 'loadPublicArchiveSemanticSearchRows', 'match_public_qa_hybrid_search', 'publicArchiveHybridSearchTerms', 'search_semantic_similarity', 'search_lexical_matches', 'semanticSearchBoost']) {
  assert(server.includes(marker), `Public hibrit arama isareti eksik: ${marker}`);
}
assert(publicRendererSource.includes('En Uygun Cevaplar') && publicRendererSource.includes('anlamca bağlantılı cevaplar'), 'Arama sayfasi anlam eslesmesini sade dille gostermeli.');
...
const askQuestionIconSource = fs.readFileSync(path.join(publicAssetRoot, 'icons/ask-question.svg'), 'utf8');
assert(askQuestionIconSource.includes('stroke-width="1.7"') && askQuestionIconSource.includes('M21 11.5'), 'Public soru CTA ikonu konusma balonlu onceki tasarimi korumali.');
for (const marker of [
...
for (const marker of [
  ...
  'publicArchiveSubmittedText(item.answerText',
  ...
  'RESEND_API_KEY',
  'sendPublicAnswerNotificationEmail',
  'sendPublicQuestionReceivedEmail',
...
assert(publicRendererSource.includes('Sorunuz başarıyla alındı. Cevap hazırlandığında size e-posta ile haber vereceğiz'), 'Public soru basari mesaji mail ve hesap takibini anlatmali.');
assert(publicRendererSource.includes('Cevap hazırlandığında size e-posta ile haber verilir'), 'Public soru form notu cevap mailini anlatmali.');
for (const marker of [
...
for (const item of publicRenderCases) {
  ...
  assert(rendered.html.includes('/public-preview/public-archive.css?v=20260922-search-route-instant-v2'), `${item.route} yalniz versiyonlu public CSS yuklemeli.`);
...
assert(rootLaunchPreview.includes('href="/public-archive.css?v=20260922-search-route-instant-v2"'), 'Root public mode versiyonlu CSS adresini root path ile uretmeli.');
...
const categoriesLaunchPreview = renderPublicArchivePreviewRoute('/kategoriler', {}, { ...publicArchiveFixtures, basePath: '', noindex: false }).html;
...
for (const marker of ['PUBLIC_ARCHIVE_STATIC_CACHE', 'PUBLIC_ARCHIVE_ASSET_VERSION', '20260922-search-route-instant-v2', "immutable: !noindex", "maxAge: noindex ? 0 : '1y'", "res.set('Cache-Control', noindex ? 'no-store, no-cache, must-revalidate, proxy-revalidate' : PUBLIC_ARCHIVE_STATIC_CACHE)"]) {
...
for (const marker of ['data-newsletter-form', 'data-newsletter-endpoint="/public-preview/api/newsletter/subscribe"', 'E-posta bülteni', 'Yeni içerikler doğrudan e-postanıza gelsin.', 'E-posta ile haberdar olun', 'Yeni içerik duyurularını e-posta ile almayı kabul ediyorum', "iconSvg('mail')", 'bindNewsletterForms', 'data-newsletter-status']) {
  assert(homePreview.includes(marker) || publicRendererSource.includes(marker), `Public abonelik formu marker eksik: ${marker}`);
}
for (const marker of ['data-cookie-banner hidden', 'data-cookie-reject', 'data-cookie-preferences', 'data-cookie-accept', 'data-cookie-modal hidden', 'dsca-cookie-consent:v1', 'bindCookieConsent', 'analyticsConsentGranted', 'clearAnalyticsStorage', 'X-Analytics-Consent', 'consentVersion: cookieConsentVersion']) {
  assert(homePreview.includes(marker) || publicRendererSource.includes(marker), `Public cerez tercihi marker eksik: ${marker}`);
}
for (const marker of ['.pa-cookie-banner', '.pa-cookie-actions', '.pa-cookie-modal', '.pa-cookie-dialog', '.pa-newsletter', '.pa-newsletter-kicker', '.pa-newsletter-form-heading', '.pa-newsletter-input-row', '.pa-footer-preferences', 'font-size: 16px']) {
  assert(publicCss.includes(marker), `Public cerez/abonelik CSS marker eksik: ${marker}`);
}
...
assert(topicArticleBodyIndex > -1 && topicArticleSupportIndex > topicArticleBodyIndex, 'Public konu rehberi destek kartlari blog govdesinden sonra gelmeli.');
assert(topicArticleRelatedIndex === -1 || topicArticleSupportIndex < topicArticleRelatedIndex, 'Public konu rehberi destek kartlari ilgili sorulardan once gelmeli.');
...
for (const marker of ['homeQuestionSets', 'uniqueHomeQuestions', 'weightedHomeScore', 'homeRotationHour', 'hashString', 'homeCollectionEntries', 'quranEvidenceEntries', 'HOME_READING_PATHS']) {
...
for (const marker of ['PUBLIC_ARCHIVE_SEO_TITLE_MAX', 'PUBLIC_ARCHIVE_SEO_DESCRIPTION_MAX', 'questionSeoTitle', 'questionSeoDescription', 'publicArchiveOrganizationStructuredData', 'collectionPageStructuredData', 'topicArticleStructuredData', 'renderTopicGuideArticle', "'@type': 'BlogPosting'", "'@type': 'Question'", "'@type': 'Answer'", 'acceptedAnswer', "'@type': 'CollectionPage'", "'@type': 'ItemList'", 'id="cevap"', 'QAPage kullanılmaz']) {
...
for (const marker of ['bindFastPublicNavigation', 'replacePublicArchiveShell', 'DOMParser', 'replaceWith', 'cleanupPublicArchivePage', '__publicArchiveFastNavBound', 'dsca-page-cache:v20', 'X-Public-Navigation', 'pushState({ paFast: true }', "window['his' + 'tory']", 'requestIdleCallback']) {
