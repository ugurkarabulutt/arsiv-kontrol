...
-- Bülten yönetim merkezi: izin kaydı Supabase'te, gönderim Resend Broadcasts'ta tutulur.
...
-- Public arşiv anlam araması. İndeks yalnız yayınlanmış public soru-cevap
-- metinlerinden sunucu tarafında üretilir; anon/authenticated doğrudan erişemez.
...
create index if not exists public_qa_search_documents_embedding_hnsw_idx
  on public.public_qa_search_documents using hnsw (embedding vector_cosine_ops);
...
as $$
  with nearest as materialized (
    ...
    where exists (
      select 1 from public.public_qa as qa
      where qa.slug = document.qa_slug and qa.status = 'published'
    )
...
create index if not exists public_qa_search_documents_text_trgm_idx
  on public.public_qa_search_documents using gin (search_text extensions.gin_trgm_ops);
...
as $$
  with terms as materialized (
    ...
    select document.qa_slug, document.document_key, document.document_kind,
      document.content as matched_text,
      1 - (document.embedding operator(extensions.<=>) p_query_embedding) as similarity,
      (select count(*)::integer from terms
       where document.search_text like ('%' || terms.term || '%')) as lexical_matches
    from public.public_qa_search_documents as document
    where exists (select 1 from public.public_qa as qa
      where qa.slug = document.qa_slug and qa.status = 'published')
    order by document.embedding operator(extensions.<=>) p_query_embedding
    ...
    select * from semantic_candidates union all select * from lexical_candidates
  ), documents as (
    select distinct on (combined.qa_slug, combined.document_key)
      combined.qa_slug, combined.document_key, combined.document_kind,
      combined.matched_text, combined.similarity, combined.lexical_matches,
    ...
    select distinct on (documents.qa_slug)
      documents.qa_slug, documents.document_kind, documents.matched_text,
      documents.similarity, documents.lexical_matches, documents.hybrid_score
    from documents
    order by documents.qa_slug, documents.hybrid_score desc
  )
  select records.qa_slug, records.document_kind, records.matched_text,
    records.similarity, records.lexical_matches, records.hybrid_score
  from records
